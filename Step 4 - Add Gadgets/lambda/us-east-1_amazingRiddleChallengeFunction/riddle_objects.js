'use strict';

module.exports = {
	LEVELS : {
		easy : [
			{
				question : "I run all day and never walk, I tell you something, but I do not talk!",
				answer : "clock",
				hints : [
					"I have two hands!",
					"You can find me on a table, on a wall, on your wrist, or on your phone!",
					"Check on me to see the time of day."
				]
			},
			{
				question : "When you look at my face it is easy to see, You're looking at you when you're looking at me!",
				answer : "mirror",
				hints : [
					"I am known for reflection!",
					"I usually live above a sink in a bathroom!",
					"Wave at yourself when you look at me!"
				]
			},
			{
				question : "What is full of holes but holds water?",
				answer : "sponge",
				hints : [
					"Wash dishes with me!",
					"I am typically yellow.",
					"A famous version of me is square pants"
				]
			},
			{
				question : "What has four fingers and one thumb but is not alive?",
				answer : "glove",
				hints : [
					"Not a hand, but warms it up!",
					"Wear me in the winter or when gardening.",
					"I usually come in a pair"
				]
			},
			{
				question : "Soft as a petal that falls from a tree, the more I dry the wetter I'll be!",
				answer : "towel",
				hints : [
					"I am with you at the beach, when you work out, or in your bathroom!",
					"See you after a nice swim!",
					"Use me when you are done in the shower."
				]
			}
		],
		medium : [
			{
				question : "I can be quick and then I'm deadly, I am a rock, shell and bone medley.",
				answer : "sand",
				hints : [
					"If I was made into a man, I'd make people dream",
					"I gather in my millions by ocean, sea and stream.",
					"Use me to sculpt a castle."
				]
			},
			{
				question : "When I point up it's bright, but when I point down it's dark. What am I?",
				answer : "light switch",
				hints : [
					"where there is a room with light, there I am",
					"I have two states",
					"I need power to function"
				]
			},
			{
				question : "Lighter than what I am made of, More of me is hidden Than is seen.",
				answer : "iceberg",
				hints : [
					"It is cold in here",
					"The titanic should look out",
					"You can put pieces of me in a drink to cool down"
				]
			},
			{
				question : "I am more useful when I am broken. What am I?",
				answer : "egg",
				hints : [
					"I am very fragile",
					"I am a future chicken",
					"I go great with bacon"
				]
			},
			{
				question : "I'm the part of the bird that's not in the sky. I can swim in the ocean and yet remain dry. What am I?",
				answer : "shadow",
				hints : [
					"Lights off, I disappear",
					"Lights on, I appear",
					"The sun and a surface are my companions"
				]
			}
		],
		hard : [
			{
				question : "The more you take the more you leave behind?",
				answer : "foot steps",
				hints : [
					"I am an imprint to where you have been",
					"I am the same size and shape",
					"You see me on sand, in rain, or when you step in something"
				]
			},
			{
				question : "What is black when you buy it, red when you use it, and gray when you throw it away?",
				answer : "charcoal",
				hints : [
					"Got something to grill? You need me",
					"You can mine me in minecraft from burnt wood",
					"I am the aftermath of a fire"
				]
			},
			{
				question : "Mountains will crumble and temples will fall, and no man can survive its endless call. What is it?",
				answer : "time",
				hints : [
					"Check your watch",
					"Check your clock",
					"No one is immune"
				]
			},
			{
				question : "I can be cracked, I can be made. I can be told, I can be played. What am I?",
				answer : "joke",
				hints : [
					"Some people find me funny",
					"Some people find me rude",
					"Anyone can take part"
				]
			},
			{
				question : "Lovely and round, I shine with pale light, grown in the darkness, A lady's delight. What am I?",
				answer : "pearl",
				hints : [
					"I live in a sealed place",
					"A clam is my home",
					"I am valuable on a neck or wrist"
				]
			}
		]
	}
};
